...
Runs daily via GitHub Actions at 08:00 AM (Taiwan Time).
Fetches factual data from APIs/RSS, synthesizes personalized insights via Gemini,
and exports `today.json` for GitHub Pages.
"""
...
# Timezone Taiwan (UTC+8)
TZ_TAIWAN = timezone(timedelta(hours=8))

def fetch_weather(cwa_api_key=None):
    """
    Fetch weather data for New Taipei City Luzhou District.
    Uses Central Weather Administration (CWA) API if CWA_API_KEY is available.
    Endpoint: https://opendata.cwa.gov.tw/api/v1/rest/datastore/F-D0047-071
    """
    print("Fetching Weather Data from CWA API...")
    if cwa_api_key:
        try:
            # CWA Open Data API (F-D0047-071: New Taipei City Township Forecast)
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                ...
                min_t = elements.get('MinT', [{}])[0].get('elementValue', [{}])[0].get('value', '28')
                max_t = elements.get('MaxT', [{}])[0].get('elementValue', [{}])[0].get('value', '34')
                pop = elements.get('PoP12h', [{}])[0].get('elementValue', [{}])[0].get('value', '40')
                wx = elements.get('Wx', [{}])[0].get('elementValue', [{}])[0].get('value', '多雲短暫雷陣雨')
                return {
                    "location": "新北市蘆洲區",
                    "condition": f"{wx} 🌤️",
...
def fetch_exchange_rate():
    """
    Fetch CNY/TWD Exchange Rate from Open Exchange API.
...
def load_quiz_questions():
    """
    Load official driving quiz questions from data/questions.json and randomly sample 5.
    Source: Ministry of Transportation and Communications Highway Bureau Official Test Bank.
    """
    print("Selecting 5 Scooter License Test Questions from Official Bank...")
    quiz_file = os.path.join(os.path.dirname(__file__), '../data/questions.json')
...
def fetch_rss_news():
    """
    Fetch raw RSS items from Tech & AI feeds.
    """
...
def synthesize_with_gemini(weather, exchange_rate, rss_items, gemini_api_key):
    """
    Call Gemini 2.5 Flash-Lite API via REST to synthesize personalized insights.
...
【請輸出嚴格的 JSON 格式 (不可包覆 Markdown 標籤之外的字串)】:
{{
...
def main():
    ...
    out_file = os.path.join(os.path.dirname(__file__), '../today.json')
